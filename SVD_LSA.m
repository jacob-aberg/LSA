...
% if vt1(maxabs(vt1)) < 0, vt1 = -1*v;     end
% if vt2(maxabs(vt2)) < 0, vt2 = -1*v;     end
% if vt3(maxabs(vt3)) < 0, vt3 = -1*v;     end
...
if nargin == 4 % om 4 argument angavs (inget z)
    for i=1:length(x)
        if i < 105, punkt = punkter{ceil(i/7)}; end
        if j > 7, j = 1; end,  color = colors(j,1:3); 
